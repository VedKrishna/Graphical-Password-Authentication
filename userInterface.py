...
class verFuns:
    def __init__(self):
        self.label = None
        self.email = None
    #To verify that the user entered email is correct
    def checkEmail(self):
        self.email = self.getEmail()
        res = verifyEmail(self.email)
        if res:
            if self.label:
                self.label.destroy()
            root.open_window()
        else:
            self.label = Label(root, text="Not a valid email")
            self.label.grid(row=4)
            

    #Checking whether the password is correct or wrong
    def submitAct(self):
        res = checkCredentials(self.email, temp.passw)
        if res:
            print("Verified!!")
            root.destroy()
        else:
            print("Wrong Password")
            print("You have " + str(2-temp.count) + " tries left")
            res = temp.clear(1)
            if res:
                root.destroy()

    def getEmail(self):
        return root.email.get()
...
class Window(Toplevel):
    def __init__(self, parent):
        ...
        img_button1 = Button(self, image=img1, command = lambda: temp.getImgId(1))
        ...
        img_button2 = Button(self, image=img2, command = lambda: temp.getImgId(2))
        ...
        img_button3 = Button(self, image=img3, command = lambda: temp.getImgId(3))
        ...
        img_button4 = Button(self, image=img4, command = lambda: temp.getImgId(4))
        ...
        img_button5 = Button(self, image=img5, command = lambda: temp.getImgId(5))
        ...
        img_button6 = Button(self, image=img6, command = lambda: temp.getImgId(6))
        ...
        img_button7 = Button(self, image=img7, command = lambda: temp.getImgId(7))
        ...
        img_button8 = Button(self, image=img8, command = lambda: temp.getImgId(8))
        ...
        img_button9 = Button(self, image=img9, command = lambda: temp.getImgId(9))
...
class Main(Tk):
    def __init__(self):
        ...
        msg = Label(self, text="Please enter your email", padx=20, pady=20)
        msg.grid(row=1)

        self.email = Entry(self)
        self.email.grid(row=2)

        subEmail = Button(self, text = "Submit", command = lambda: temp2.checkEmail(), pady = 20)
        subEmail.grid(row = 3, pady=20)
